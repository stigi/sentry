    ...
    def has_for_batch(
        ...
        try:
            ...
            if self._entity_handler and unresolved_projects:
                feature_type = self._get_feature_class(name)
                with metrics.timer("features.entity_batch_has", sample_rate=0.01):
                    if issubclass(feature_type, ProjectFeature):
                        projects = [
                            project for project in objects if project in unresolved_projects
                        ]
                        ...
                        if entity_results:
                            for project in projects:
                                value = entity_results.get(f"project:{project.id}", {}).get(name)
                                if value is not None:
                                    result[project] = value
                                    unresolved_projects.remove(project)
                    else:
                        entity_results = self._entity_handler.batch_has(
                            [name], actor, organization=organization
                        )
                        if entity_results:
                            value = entity_results.get(f"organization:{organization.id}", {}).get(
                                name
                            )
                            if value is not None:
                                for project in unresolved_projects:
                                    result[project] = value
                                unresolved_projects.clear()
