...
def _build_comparison_plan(
    ...
    if sibling is not None and diff_sibling_images:
        ...
        for name in sorted(possibly_interesting):
            ...
            if not sibling_hash or sibling_hash == head_hash:
                continue
            head_meta = head_meta_by_hash[head_hash]
            pixel_count = ImageSize(head_meta.width, head_meta.height).pixel_count
            if pixel_count > MAX_DIFF_PIXELS:
                continue
...
def compare_snapshots(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        diff_sibling_images = features.has(
            "organizations:preprod-snapshot-auto-approve-sibling-diffs",
