...
import {useDraftableQueryState} from 'sentry/views/dashboards/widgetBuilder/hooks/useDraftableQueryState';
import {
  DISABLED_SORT,
  TAG_SORT_DENY_LIST,
...
export function useWidgetBuilderState(): {
  ...
  const [title, setTitle] = useDraftableQueryState('title', parseAsString);
  const [description, setDescription] = useDraftableQueryState(
    'description',
    parseAsString
  );
  const [displayType, setDisplayType] = useDraftableQueryState(
    'displayType',
    parseAsDisplayType
  );
  const [dataset, setDataset] = useDraftableQueryState('dataset', parseAsDataset);
  ...
  const [fields, setFields] = useDraftableQueryState('field', parseAsColumns);
  const [yAxis, setYAxis] = useDraftableQueryState('yAxis', parseAsColumns);
  const [query, setQuery] = useDraftableQueryState('query', parseAsQueries);
  const [sort, setSort] = useDraftableQueryState('sort', parseAsWidgetSorts(datasetRef));
  const [limit, setLimit] = useDraftableQueryState('limit', parseAsLimit);
  const [legendAlias, setLegendAlias] = useDraftableQueryState(
    'legendAlias',
    parseAsStringList
  );
  const [legendType, setLegendType] = useDraftableQueryState(
    'legendType',
    parseAsLegendType
  );
  const [selectedAggregate, setSelectedAggregate] = useDraftableQueryState(
    'selectedAggregate',
    parseAsSelectedAggregate
  );
  const [thresholds, setThresholds] = useDraftableQueryState(
    'thresholds',
    parseAsThresholds
  );
  const [linkedDashboards, setLinkedDashboards] = useDraftableQueryState(
    'linkedDashboards',
    parseAsLinkedDashboards
  );
  const [axisRange, setAxisRange] = useDraftableQueryState('axisRange', parseAsAxisRange);
