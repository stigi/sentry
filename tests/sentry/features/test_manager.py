...
class FeatureManagerTest(TestCase):
    ...
    def test_has_for_batch_capture_error(self) -> None:
        ...
        partial_handler = mock.Mock(spec=features.BatchFeatureHandler)
        partial_handler.features = {"organizations:faulty"}
        partial_handler.has_for_batch.return_value = {project: True, other_project: None}

        failing_handler = mock.Mock(spec=features.BatchFeatureHandler)
        failing_handler.features = {"organizations:faulty"}
        failing_handler.has_for_batch.side_effect = ValueError("invalid thing")
        ...
        with (
            ...
            res = manager.has_for_batch("organizations:faulty", org, [project, other_project])
            assert res == {project: True, other_project: False}
            assert mock_capture.call_count == 1
    ...
    def test_has_for_batch_uses_organization_entity_result(self) -> None:
        ...
        entity_handler.batch_has.assert_called_once_with(
            [feature_name], self.user, organization=self.organization
        )
